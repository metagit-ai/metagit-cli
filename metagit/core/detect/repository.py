...
class ProjectTypeDetection(BaseModel):
    ...
    domain: ProjectDomain = Field(default=ProjectDomain.OTHER, description="Project domain")
...
class RepositoryAnalysis(BaseModel):
    ...
    def _load_existing_workspace(self) -> None:
        ...
        if os.path.exists(config_path):
            ...
            try:
                ...
                self.logger.warning(f"Failed to load or parse existing .metagit.yml: {e}")
        ...
        try:
            ...
            for root, dirs, files in os.walk(self.path):
                ...
                for file in files:
                    ...
                    # Check build tools
                    if file in build_files_data.get('build_files', {}).get('general', []):
                    ...
                    if file.endswith('.py'):
                        ...
                        if 'angular' in file.lower() or 'angular' in relative_path.lower():
                    ...
                    elif file.endswith('.cpp') or file.endswith('.cc') or file.endswith('.cxx'):
                        detected_languages.add('C++')
            ...
            if detected_languages:
                ...
                language_priority = {
                    'Python': 10, 'JavaScript': 9, 'Go': 8, 'Rust': 7,
                    'Java': 6, 'C#': 5, 'PHP': 4, 'Ruby': 3, 'C++': 2, 'C': 1
                }
                primary_language = max(detected_languages, key=lambda x: language_priority.get(x, 0))
    ...
    def _detect_project_type(self) -> Union[ProjectTypeDetection, Exception]:
        ...
        try:
            ...
            if self.language_detection:
                ...
                # Web applications
                elif any(f.endswith(('.html', '.js', '.ts', '.jsx', '.tsx')) for f in all_files):
                    project_type = ProjectType.APPLICATION
                    domain = ProjectDomain.WEB
                    confidence = 0.7
                
                # Libraries
                elif self.language_detection.primary in ['Python', 'JavaScript', 'Go', 'Rust']:
                    # Check for library indicators
                    if any(f in ['setup.py', 'pyproject.toml', 'package.json', 'Cargo.toml'] for f in all_files):
                ...
                # Data science
                    ...
                    if any('data' in f.lower() or 'ml' in f.lower() or 'ai' in f.lower() for f in all_files):
        ...
        try:
            ...
            for root, dirs, files in os.walk(self.path):
                ...
                for file in files:
                    ...
                    # Testing files
                    elif any(test in file.lower() for test in ['test', 'spec', 'jest', 'pytest', 'unittest']):
                        files_by_category['testing'].append(relative_path)
                        self.has_tests = True
                    
                    # Documentation files
                    elif any(doc in file.lower() for doc in ['readme', 'docs', 'documentation', '.md']):
                        files_by_category['documentation'].append(relative_path)
                        self.has_docs = True
                    
                    # Infrastructure as Code
                    elif file.endswith(('.tf', '.yaml', '.yml')) and any(iac in file.lower() for iac in ['terraform', 'kubernetes', 'helm', 'ansible']):
                        files_by_category['iac'].append(relative_path)
                        self.has_iac = True
                    
                    # CI/CD files
                    elif any(ci in file.lower() for ci in ['.github', '.gitlab', 'jenkins', 'travis', 'circle']):
                        files_by_category['ci_cd'].append(relative_path)
                    
                    # Build files
                    elif any(build in file.lower() for build in ['makefile', 'build', 'gradle', 'maven', 'ant']):
                        files_by_category['build'].append(relative_path)
                    
                    # Config files
                    elif any(config in file.lower() for config in ['.config', '.conf', '.ini', '.json', '.yaml', '.yml']):
    ...
    def _extract_metadata(self) -> None:
        ...
        try:
            ...
            license_files = ['LICENSE', 'LICENSE.txt', 'LICENSE.md', 'license', 'COPYING']
                ...
                if os.path.exists(license_path):
                    try:
                        with open(license_path, 'r', encoding='utf-8') as f:
                            ...
                            if 'mit' in content:
                                self.license_info = License(kind=LicenseKind.MIT, file=license_file)
                            elif 'apache' in content:
                                self.license_info = License(kind=LicenseKind.APACHE_2_0, file=license_file)
                            elif 'gpl' in content:
                                self.license_info = License(kind=LicenseKind.GPL_3_0, file=license_file)
                            elif 'bsd' in content:
                                self.license_info = License(kind=LicenseKind.BSD_3_CLAUSE, file=license_file)
                            else:
                                self.license_info = License(kind=LicenseKind.NONE, file=license_file)
                            break
                    except Exception:
                        continue
    ...
    def _load_package_manager_data(self) -> Union[Dict[str, str], Exception]:
        ...
        try:
            data_path = os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'package-managers.json')
    ...
    def _load_build_files_data(self) -> Union[Dict[str, Any], Exception]:
        ...
        try:
            data_path = os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'build-files.yaml')
            with open(data_path, 'r') as f:
    ...
    def _detect_metrics(self) -> None:
        ...
        try:
            ...
            # In a real implementation, you'd want to make API calls to get these
            stars = 0
            forks = 0
            open_issues = 0
            open_prs = 0
            
            # Try to get remote URL to determine if we can fetch metrics
            if repo.remotes:
                origin = next((r for r in repo.remotes if r.name == "origin"), None)
                if origin and origin.urls:
                    remote_url = next(iter(origin.urls), None)
                    if remote_url:
                        self.logger.debug(f"Remote URL found: {remote_url}")
                        # Here you could add API calls to GitHub/GitLab to get actual metrics
                        # For now, we'll use the git-based metrics we can calculate
            ...
            self.logger.debug(f"Detected metrics: {len(contributors)} contributors, {commit_frequency.value} commit frequency")
    ...
    def to_metagit_config(self) -> Union[MetagitConfig, Exception]:
        ...
        try:
            ...
            if self.branch_analysis and self.branch_analysis.strategy_guess:
                ...
                branch_strategy = strategy_map.get(self.branch_analysis.strategy_guess, BranchStrategy.NONE)
            ...
            if self.ci_config_analysis and self.ci_config_analysis.detected_tool:
                ...
                platform = platform_map.get(self.ci_config_analysis.detected_tool, CICDPlatform.CUSTOM)
                cicd_config = CICD(
                    platform=platform,
                    pipelines=[
                        Pipeline(
            ...
            metadata = RepoMetadata(
                default_branch=self.branch_analysis.branches[0].name if self.branch_analysis and self.branch_analysis.branches else None,
                has_ci=self.ci_config_analysis is not None,
                has_tests=self.has_tests,
                has_docs=self.has_docs,
                has_docker=self.has_docker,
                has_iac=self.has_iac,
                created_at=datetime.now(),  # Would need git history for actual creation date
                last_commit_at=datetime.now()  # Would need git history for actual last commit
            )
            ...
            return MetagitConfig(
                ...
                url=self.url,
                kind=self.project_type_detection.type if self.project_type_detection else ProjectKind.OTHER,
    ...
    def summary(self) -> Union[str, Exception]:
        ...
        try:
            lines = [f"Repository Analysis Summary"]
            ...
            if self.language_detection:
                ...
                if self.language_detection.package_managers:
                    lines.append(f"Package Managers: {', '.join(self.language_detection.package_managers)}")
            
            if self.project_type_detection:
                lines.append(f"Project Type: {self.project_type_detection.type.value}")
                lines.append(f"Domain: {self.project_type_detection.domain.value}")
                lines.append(f"Confidence: {self.project_type_detection.confidence:.2f}")
            ...
            if self.metrics:
                ...
                lines.append(f"Commit Frequency: {self.metrics.commit_frequency.value}")
                ...
                lines.append(f"PRs Merged (30d): {self.metrics.pull_requests.merged_last_30d}")
