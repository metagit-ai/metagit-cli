...
- **Resources:** Start with `metagit://catalog`. Phase 1–4 URIs include `gate/status` (includes `state_backend` diagnostics), `workspace/map?limit=80`, `session/meta`, `session/digest`, `objectives`, `approvals/pending`, `handoffs/open`, `events/recent`, `prompt/catalog`, dynamic `prompt/{scope}/{kind}`, `project/{project}/summary`, `repo/{project}/{repo}/card`, `workspace/config` (summary default; `?view=full` refused above 80 repos unless `confirm=1`), `workspace/repos/status?summary=1`, `workspace/health`, `workspace/context` (alias). Spec: [mcp-layered-resources-spec.md](reference/mcp-layered-resources-spec.md). Large umbrellas: [context-reduction.md](reference/context-reduction.md). Skills: `metagit-mcp-resources`, `metagit-sharing-state`.
- **Campaigns (active gate):** `metagit_campaign_status` (paged overlay summary), `metagit_campaign_board_sync` (Azure Boards WIT, `dry_run` first), `metagit_campaign_context`. Do not return the frozen `_campaigns/*.yml` document to the model.
- **Organization index (active gate):** `metagit_org_index` (`organization`, optional `refresh`/`full`), `metagit_org_search` (query/language/topic/has/stale_days), `metagit_org_repo_get`, `metagit_graph_neighbors`, `metagit_org_repo_materialize` (`confirm: true` unless `dry_run`). CLI: `metagit org index github`, `metagit org search`, `metagit graph neighbors`, `metagit repo materialize`. Observations stay in `~/.metagit/indexes/github/`; curated edges stay in `.metagit.yml` `graph`. See [org-index.md](reference/org-index.md).
