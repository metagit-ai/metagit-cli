...
### Added
- Context reduction for large umbrellas: catalog/map/pack/digest/campaign JSON is paged; `workspace list` omits the full `workspace` object unless `--include-workspace`; `config show --json` and `metagit://workspace/config?view=full` refuse above 80 repos unless `--confirm-full` / `confirm=1`. Prompt kind `large-workspace`. Docs: [context-reduction.md](reference/context-reduction.md).
...
### Security
- Bump transitive `soupsieve` to `>=2.9.2` (CVE-2026-85999 / CVE-2026-86000).
