...
- If you encounter a query parsing error (422), check for disallowed special characters and ensure the query includes at least one valid search term.
    ...
    def handle_rate_limit(self, gh: Github, query: str) -> str:
        ...
        local_reset_timestamp: datetime = time.mktime(
            self.utc_to_local(gh.get_rate_limit().search.reset).timetuple()
        )
        print(f"Time until reset: {local_reset_timestamp - current_time} seconds")

        sleep_time = (
            local_reset_timestamp - current_time + 10
        )  # adding 10 seconds to ensure the limit is reset
        ...
        if sleep_time > 0:
            ...
            print(
                f"Calculated negative sleep time: {sleep_time} seconds. Reset time might have already passed."
            )
