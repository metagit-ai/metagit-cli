...
_DSL_RE = re.compile(r"^(?P<kind>[A-Za-z][A-Za-z0-9_]*)\[id=(?P<quote>[\"'])(?P<id>[^\"']+)(?P=quote)\]\s*$")
...
class AtlasQuery:
    ...
    def list_entities(self, *, kind: str | None = None) -> AtlasQueryResult | Exception:
        ...
        try:
            ...
            if isinstance(entities, Exception):
                return entities
            filtered = [entity.model_dump(mode="json") for entity in entities if kind is None or entity.kind == kind]
