...
class GitCacheEntry(BaseModel):
    ...
    local_commit_hash: Optional[str] = Field(None, description="Current local commit hash")
    local_branch: Optional[str] = Field(None, description="Current local branch name")
    remote_commit_hash: Optional[str] = Field(None, description="Latest remote commit hash")
    remote_branch: Optional[str] = Field(None, description="Default remote branch name")
    has_upstream_changes: Optional[bool] = Field(None, description="Whether upstream has new commits")
    upstream_changes_summary: Optional[str] = Field(None, description="Summary of upstream changes")
    last_diff_check: Optional[datetime] = Field(None, description="Last time differences were checked")
...
class GitCacheConfig(BaseModel):
    ...
    default_timeout_minutes: int = Field(default=60, description="Default cache timeout in minutes")
    ...
    git_config: Dict[str, Any] = Field(default_factory=dict, description="Git configuration options")
    provider_config: Optional[Dict[str, Any]] = Field(None, description="Provider-specific configuration")
    entries: Dict[str, GitCacheEntry] = Field(default_factory=dict, description="Cache entries")
    ...
    def get_cache_size_bytes(self) -> int:
        ...
        for entry in self.entries.values():
            if entry.cache_path.exists():
                try:
                    total_size += sum(f.stat().st_size for f in entry.cache_path.rglob("*") if f.is_file())
