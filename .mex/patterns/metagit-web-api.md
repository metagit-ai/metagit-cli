...
- Ops routes live in `OpsWebHandler` (`src/metagit/core/web/ops_handler.py`): POST `/v3/ops/health`, `/v3/ops/prune/preview`, `/v3/ops/prune`, `/v3/ops/sync`, `/v3/ops/source-sync`; GET `/v3/ops/sync/{job_id}`; GET `/v3/ops/approvals`; POST `/v3/ops/approvals/{id}/resolve`; GET `/v3/ops/sync/{job_id}/events` (SSE); GET `/v3/ops/components/graph` (RFC-0028), GET `/v3/ops/components/detect` (RFC-0031, register before `/v3/ops/components`), GET `/v3/ops/components` and GET `/v3/ops/components/resolve` (RFC-0027 `ComponentResolver`), POST `/v3/ops/components/init` (RFC-0031). Approval resolve uses `ApprovalResolveOrchestrator` (same as CLI). Module-level `SyncJobStore` tracks async sync jobs.
