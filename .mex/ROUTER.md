...
- **Metagit Web:** local `metagit web serve` + packaged SPA (**Config Studio** on `/config/*`, **Workspace Console** on `/workspace` with repo table, **Explorer** tree view with tag-aware filters and open-in-editor actions, **content grep** search tab, graph view, ops panel) with `task web:dev` / `task web:build` workflow documented in [`docs/reference/metagit-web.md`](../docs/reference/metagit-web.md).
- **Agents Web console:** `/agents` now includes `Templates`, `Objectives`, and `Sessions` sub-tabs; Objectives uses `/v3/ops/objectives*` for grouped editing with prominent agent notes, Sessions uses `/v3/ops/session*` for digest + begin-session actions, and both support opt-in polling with a default 90s cadence.
