...
- **`metagit fmt` / `metagit format`:** schema-ordered, readable YAML for `.metagit.yml` and `metagit.config.yaml`; preserves comments, injects yaml-language-server schema directive, 2-space indent (`ConfigFormatService`, `yaml_roundtrip`, `yaml_order`).
