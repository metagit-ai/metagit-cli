...
- **RFC-0010 + RFC-0011 (in progress on `feat/rfc-0010-0011`):** RFC-0010 Semantic KG is implemented through Task 10 closeout: `metagit.core.semantic` models/paths/store/service/events, declare/query/owners/conflicts/seed/ingest, advisory ACL `concept_hints`, semantic events in `metagit context events`, `metagit semantic declare|query|owners|conflicts|ingest|seed`, ACTIVE-gated MCP `metagit_semantic_*` tools, `semantic_ownership` modality parity, operator docs, agent quick references, and GROW pattern updates. Task 9 `semantic ingest --gitnexus` is deferred/optional. RFC-0011 Tasks 1-3 are implemented: `metagit.core.merge` models/paths/store, GitPython `git_ops`, `MergeOrchestrator`, merge events, clean merge success records, and conflict records with ACL hint commands only. Worktree `.worktrees/rfc-0010-0011`.
