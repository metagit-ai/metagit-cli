...
- **RFC-0010 + RFC-0011 (in progress on `feat/rfc-0010-0011`):** Semantic KG plan expanded (TDD); Task 1 foundation (`metagit.core.semantic` models, paths, JSON store) implemented; Task 2 `SemanticGraphService` declare/query/owners implemented with `ConceptDeclared` events; Task 3 advisory `conflicts(repository)` concept hints implemented against active ACL claims with `ConceptConflictHint` events; Task 4 merges semantic lifecycle events into `metagit context events` as `source=semantic`; Task 5 attaches non-blocking semantic `concept_hints` to ACL claim checks; Task 6 adds `metagit semantic declare|query|owners|conflicts`; Task 7 adds ACTIVE-gated MCP `metagit_semantic_*` tools plus `semantic_ownership` modality parity markers; Task 8 implements optional static seed catalog plus deterministic ingest hints with `ConceptIngested` events. Worktree `.worktrees/rfc-0010-0011`.
