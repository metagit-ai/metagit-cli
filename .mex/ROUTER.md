...
**Working:**
- **RFC-0008 Task Graph & Intent + RFC-0009 Context Compiler (implemented; merge pending):** `metagit task` + MCP `metagit_task_*`; `metagit context compile` + MCP `metagit_context_compile`; packages `metagit.core.taskgraph` / `metagit.core.context.compiler`; docs `task-graph.md` / `context-compiler.md`; modality `task_graph` / `context_compile`.
- **RFC-0010 + RFC-0011 (implemented on `feat/rfc-0010-0011`):** RFC-0010 Semantic KG is implemented through Task 10 closeout: `metagit.core.semantic` models/paths/store/service/events, declare/query/owners/conflicts/seed/ingest, advisory ACL `concept_hints`, semantic events in `metagit context events`, `metagit semantic declare|query|owners|conflicts|ingest|seed`, ACTIVE-gated MCP `metagit_semantic_*` tools, `semantic_ownership` modality parity, operator docs, agent quick references, and GROW pattern updates. Task 9 `semantic ingest --gitnexus` is deferred/optional. RFC-0011 Merge Orchestrator is implemented: `metagit.core.merge` models/paths/store/git_ops/service/events/validators, clean conflict aborts with ACL hint commands only, opt-in validators and gated promote, merge events in `metagit context events`, `metagit merge enqueue|status|integrate|retry|promote`, ACTIVE-gated MCP `metagit_merge_*` tools, `merge_orchestrator` modality parity, operator docs, and GROW pattern updates. Worktree `.worktrees/rfc-0010-0011`.
- **RFC-0007 ACL foundation:** `metagit branch|lease|worktree|claim` + MCP tools; persistence under `.metagit/{branches,leases,worktrees,claims,agents,events}/`; isolated checkouts under configurable `workspace.worktrees_path` (default `.worktrees/`); advisory claims; ACL events in context feed; docs `docs/reference/agent-coordination.md`; bundled skill **`metagit-agent-coordination`**.
- **ACL RFC series (0008–0013) build specs drafted:** designs + plans under `docs/superpowers/` — index [docs/superpowers/specs/2026-07-09-acl-rfc-series-index.md](../docs/superpowers/specs/2026-07-09-acl-rfc-series-index.md). 0009–0013 phased; 0013 is composition-only (no new engines). Root `spec.md` retired; vision lives in agent-coordination + series index.
- **Import guard:** `metagit/__init__.py` prepends interpreter purelib on `sys.path` so caller `PYTHONPATH` cannot shadow bundled deps (pydantic).
- Core CLI command surface (`config`, `detect`, `project`, `record`, `workspace`, `mcp`, `search` / `find`, `api serve` for local JSON v1 search + **v2 catalog CRUD**, `project repo prune` for sync-folder cleanup, **`project repo promote`** to migrate path-based entries into git-managed clones) with shared app config + logger bootstrapping.
...
- **`task docs:links`** validates markdown links in `README.md` and `docs/**/*.md` via lychee (`scripts/check-doc-links.zsh`, `lychee.toml`); CI runs the same check on Ubuntu in `.github/workflows/test.yaml`. **`task qa:prepush`** runs a context-aware subset (changed docs + registry when modality YAML changes; skipped for src-only diffs). `docs/index.md` is a separate MkDocs home page (not a symlink to README) with docs-relative asset paths. **MkDocs `--strict`** (`.github/workflows/docs.yaml`) requires links to repo files outside `docs/` to use GitHub blob URLs — modality registry generator emits `scripts/modality-parity.yml` that way.
...
- Provider source sync is available via `metagit project source sync` for GitHub org/user and GitLab group recursive discovery with discover/additive/reconcile modes, glob filters, `--ensure`/`--refresh-metadata`, topic enrichment, `--json`, `--sync`, MCP `metagit_project_source_sync`, and `metagit workspace import` alias. **Declarative manifest sync:** `workspace.projects[].sources[]` with `source_id` on repos; `--from-manifest`, `--write-source`, `--refresh-sources`, approval-gated reconcile removals (`context approval approve`), and `SourceManifestSyncService`. **Web parity:** `POST /v3/ops/source-sync`, approvals panel, shared `ApprovalResolveOrchestrator`. **Modality parity gate:** `scripts/modality-parity.yml` checked in `task qa:prepush`.
...
- **Reference rewrite workspace:** `metagit-rewrite` init template, example at `examples/metagit-rewrite/`, guide at `docs/metagit-rewrite-workspace.md`, skill **`metagit-rewrite-campaign`** (campaign + parity registry + orchestrator script).
- **`metagit init`:** bundled init templates (`application`, `umbrella`, `hermes-orchestrator`, **`metagit-rewrite`**) with copier-style `{{ var }}` rendering, `--answers-file`, `--no-prompt`, all `ProjectKind` values via `--minimal`; idempotent when a valid `.metagit.yml` already exists (`--force` to overwrite).
