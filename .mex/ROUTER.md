...
- Provider source sync is available via `metagit project source sync` for GitHub org/user and GitLab group recursive discovery with discover/additive/reconcile modes, glob filters, `--ensure`/`--refresh-metadata`, topic enrichment, `--json`, and namespaced GitLab naming.
