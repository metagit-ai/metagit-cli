...
**Working:**
- **RFC-0012 Distributed Agent Scheduler (implemented on `feat/rfc-0012-agent-scheduler`):** `metagit.core.scheduler` scores ready task nodes (priority/affinity/cost/optional fairness + soft merge backpressure); `metagit schedule policy|next|status`; ACTIVE-gated MCP `metagit_schedule_*`; events `source=scheduler`; modality `agent_scheduler`; docs `docs/reference/agent-scheduler.md`; worktree `.worktrees/rfc-0012`.
...
- **ACL RFC series (0008–0013):** designs + plans under `docs/superpowers/` — index [docs/superpowers/specs/2026-07-09-acl-rfc-series-index.md](../docs/superpowers/specs/2026-07-09-acl-rfc-series-index.md). Next composition RFC is 0013 (no new engines). Root `spec.md` retired; vision lives in agent-coordination + series index.
